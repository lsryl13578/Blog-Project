...
@SpringBootTest // 테스트용 애플리케이션임을 명시해준다.
class BlogApiControllerTest {

	protected MockMvc mockMvc;
	
	@Autowired
	protected ObjectMapper objectMapper; // ObjectMapper: 직렬화, 역직렬화를 위한 클래스
	...
	public void addArticle() throws Exception {
		...
		assertThat(articles.size()).isEqualTo(1); // 크기가 1인지 검증(article 전체 데이터가 1개인지 검증)
		assertThat(articles.get(0).getTitle()).isEqualTo(title); // 인덱스가 0인 게시글의 제목이 "title"인지 검증
		assertThat(articles.get(0).getContent()).isEqualTo(content); // 인덱스가 0인 게시글의 내용이 "content"인지 검증
	}
